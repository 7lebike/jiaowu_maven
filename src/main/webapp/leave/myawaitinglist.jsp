	...
	<div id="wrapper" style="background: #2f4050;min-width:1100px">
		...
		<div class="gray-bg dashbard-1" id="page-wrapper">
	    	...
	    	<div class="margin-nav" style="width:100%;">	
	    		<form action="/leave/myAwaiting" method="post" id="searchForm">
					<div class="col-lg-12">
					...
					<div class="col-lg-12">
						<div class="ibox float-e-margins">
							<div class="ibox-title">
								<c:if test="${paramMap['_query.queryType'] ne '1' }">
									<h5>待我审批</h5>
									<input type="button" onclick="viewMyApplicationLists(1)" 
										value="审批记录" class="btn btn-xs btn-primary m-l-xl " />
								</c:if>
								<c:if test="${paramMap['_query.queryType'] eq '1' }">
									<h5>审批记录</h5>
									<input type="button" onclick="viewMyApplicationLists(0)" 
										value="待我审批" class="btn btn-xs btn-primary m-l-xl " />
								</c:if>
								<a class="btn btn-xs btn-success" href="/leave/list" >请假记录</a>
							</div>
							<div class="ibox-content">
								<table width="80%" class="table table-striped textcenter" >
									...
									<tbody>
										<c:forEach items="${splitPage.page.list }" var="asking" varStatus="index" >
											<tr>
												<td>${index.count }</td>
												<td><fmt:formatDate value="${asking.createdate }" pattern="yyyy-MM-dd" /> </td>
												<td>${asking.stuName }</td>
												<td>${asking.type }</td>
												<td><fmt:formatDate value="${asking.starttime }" pattern="yyyy-MM-dd HH:mm" /></td>
												<td><fmt:formatDate value="${asking.endtime }" pattern="yyyy-MM-dd HH:mm" /></td>
												<td class="askingState" stateCode="${asking.stateCode }" ></td>
												<td>
													<a href="/leave/viewAskingDetail/${asking.id }" class="btn btn-info btn-xs" >详情</a>
													<c:if test="${paramMap['_query.queryType'] ne '1' }"  >
														<c:if test="${operator_session.qx_leaveapproval }">
														<a href="/leave/approval/${asking.reviewId }" class="btn btn-xs btn-danger" >审批</a>
														</c:if>
													</c:if>
												</td>
											</tr>
